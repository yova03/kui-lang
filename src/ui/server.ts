...
export function createCompilerUiServer(root: string): http.Server {
  return http.createServer(async (request, response) => {
    try {
      const url = new URL(request.url ?? "/", `http://${request.headers.host ?? "localhost"}`);
      if (request.method === "GET" && url.pathname === "/") return sendHtml(response, pageHtml());
      if (request.method === "GET" && url.pathname === "/api/examples") return sendJson(response, examplesPayload(root));
      if (request.method === "GET" && url.pathname === "/api/inspect") return sendJson(response, inspectPayload(root, url.searchParams.get("file")));
      if (request.method === "POST" && url.pathname === "/api/compile") return sendJson(response, await compilePayload(root, request));
      if (request.method === "GET" && url.pathname === "/artifact") return sendArtifact(root, response, url.searchParams.get("file"));
      if (request.method === "GET" && url.pathname === "/api/events") return handleEvents(root, request, response, url);
...
function handleEvents(root: string, request: http.IncomingMessage, response: http.ServerResponse, url: URL): void {
  const relative = url.searchParams.get("file");
  if (!relative) return sendJson(response, { error: "Falta file." }, 400);
  let absolute: string;
  try {
    absolute = safeResolve(root, relative);
  } catch (error) {
    return sendJson(response, { error: error instanceof Error ? error.message : String(error) }, 400);
  }
  if (!existsSync(absolute) || !statSync(absolute).isFile()) {
    return sendJson(response, { error: "No existe el archivo." }, 404);
  }
  ...
  response.write(`event: ready\ndata: ${JSON.stringify({ file: relative })}\n\n`);
  const session = new WatchSession(absolute, () => {
    response.write(`event: changed\ndata: ${JSON.stringify({ file: relative, at: Date.now() })}\n\n`);
  });
...
function examplesPayload(root: string): { examples: string[] } {
  const examples = fg.sync("examples/**/*.kui", { cwd: root, onlyFiles: true }).sort();
  return { examples };
}

function inspectPayload(root: string, file: string | null): unknown {
...
async function compilePayload(root: string, request: http.IncomingMessage): Promise<unknown> {
...
function sendArtifact(root: string, response: http.ServerResponse, file: string | null): void {
...
function safeResolve(root: string, relativeOrAbsolute: string): string {
...
<body>
  <header>
    ...
    <div>
      ...
      <p class="muted" id="live-status" style="margin: 8px 0 0; font-size: 12px;"></p>
    </div>
  </header>
  <main>
    <aside>
  ...
  <script>
    const state = { file: "", inspect: null, live: null, reloading: false, pendingReload: false };
    ...
    function subscribe() {
      if (state.live && state.live.file === state.file) return;
      if (state.live) state.live.source.close();
      const source = new EventSource("/api/events?file=" + encodeURIComponent(state.file));
      state.live = { file: state.file, source };
      source.addEventListener("open", () => { liveStatus.textContent = "Live reload activo"; });
      source.addEventListener("error", () => { liveStatus.textContent = "Reconectando..."; });
      source.addEventListener("changed", scheduleReload);
    }

    function scheduleReload() {
      if (reloadTimer) clearTimeout(reloadTimer);
      reloadTimer = setTimeout(runReload, 100);
    }
    ...
    async function runReload() {
      ...
      try {
        await inspect();
        await compilePdf({ keepTab: true });
      } catch (error) {
        liveStatus.textContent = "Error al recompilar: " + (error.message || error);
      } finally {
